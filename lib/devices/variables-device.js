...
class VariablesDevice extends Device {
  ...
  async setVariable(variable_obj, path_obj) {
    ...
    if (val === null) {
      if (this.properties.has(`variable-${variable_obj.id}`))
        this.properties.delete(`variable-${variable_obj.id}`);
    } else {
      this.properties.set(`variable-${variable_obj.id}`, new VariableValueProperty(this, variable_obj, path_obj, val));
    }
