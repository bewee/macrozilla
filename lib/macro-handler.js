...
class MacroHandler {
  ...
  async notifyLoad(macro_id) {
    ...
    for (const block of description) {
      ...
      if (block.type in this.classInstances) {
        try {
          if (instance.onLoad) instance.onLoad(ctx, block);
        } catch (ex) {
          this.log(ctx, 'fatal', {title: 'Failed to load block', message: ex.message, stack: ex.stack});
        }
      } else {
  ...
  async notifyUnload(macro_id) {
    ...
    for (const block of description) {
      ...
      if (block.type in this.classInstances) {
        try {
          if (instance.onUnload) instance.onUnload(macro_id, block);
        } catch (ex) {
          this.log(ctx, 'fatal', {title: 'Failed to unload block', message: ex.message, stack: ex.stack});
        }
      } else {
        this.log(ctx, 'error', {title: 'Unknown class'});
      }
