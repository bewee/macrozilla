...
class MacroHandler {
  ...
  async init() {
    ...
    for (const f of fs.readdirSync(classesPath)) {
      try {
        this.classes[f] = require(`../classes/${f}/server.js`);
        if ('init' in this.classes[f]) await this.classes[f].init(this);
        console.info('Loaded class', f);
      } catch (ex) {
        console.info('Failed to load class', f, ':', ex);
      }
    }
    ...
    const log = (inf, status, description) => {
      ...
      if (this.macrozilla.config.macrolog2console)
        console.info(`[${Object.keys(levels).find((s) => levels[s]==status)}] Macro ${inf.macro_id}${'block_id' in inf ? `, block ${inf.block_id}` : ''}: ${JSON.stringify(description, null, 2)}`);
  ...
  async execMacro(macro_id, reason = 'Unknown') {
    ...
    if (!macro) throw Error(`Unknown macro ${macro_id}`);
    try {
      ...
      if (typeof ex === 'string') {
        ctx.log.f({title: 'Something went wrong during macro execution', message: ex});
      } else {
