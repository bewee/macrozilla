...
class DBHandler {
  ...
  constructor(apihandler) {
    ...
    this.path = `${apihandler.userProfile.dataDir}/${apihandler.packageName}/db.sqlite3`;
    this.conn = null;
  }
  ...
  updateVariableValue(id, value) {
    return new Promise((resolve, reject) => {
      ...
      `, [value, id], (err) => {
        if (err) reject(err);
        this.apihandler.eventhandler.emit('variableValueChanged', id, value);
        resolve();
      });
    });
  }
  ...
  limitLogs(macro_id = null) {
    return new Promise((resolve, _reject) => {
      new Promise((resolve, _reject) => {
        ...
        `, [this.apihandler.config.log_limit_time], resolve);
      }).then(() => {
        if (isNaN(macro_id))
          resolve();
        this.conn.run(`
        ...
        `, [macro_id, this.apihandler.config.log_limit_num], resolve);
      });
    });
  }
