...
class DBHandler {
  ...
  init() {
    return new Promise((async (resolve, reject) => {
      ...
      this.conn.serialize(() => {
        this.conn.run('PRAGMA main.user_version = ?', [], (err) => {
          if (err) reject(err);
        });
  ...
  convertDatabase(old_version, current_version) {
    return new Promise((resolve, _reject) => {
      ...
      if (!fs.existsSync(`${this.base_path}/backups`)) {
        fs.mkdirSync(`${this.base_path}/backups`);
      }
      fs.copyFileSync(this.path, `${this.base_path}/backups/db_v${old_version}.sqlite3`);
